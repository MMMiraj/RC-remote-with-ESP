...
void onDataRecv(const esp_now_recv_info_t *recvInfo, const uint8_t *incomingData, int len) {
  if (len != sizeof(RCPacket)) {
    // TX and RX packet struct sizes don't match -> one board is running
    // older firmware. Every packet gets silently dropped, and outputs
    // freeze on whatever they last were. Reflash BOTH boards with the
    // same version of the sketches to fix this.
    Serial.print("RX: packet size mismatch! got ");
    Serial.print(len);
